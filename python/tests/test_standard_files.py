...
class StandardFilesMixin(object):
    ...
    def test_bad_magic_number(self): self.assertRaises(
    ...
    def test_key_offset_outside_file(self):
        self.assertRaises(
            kas.FileFormatError, self.read_file,
            "malformed/key_offset_outside_file.kas")
    ...
    def test_key_len_outside_file(self):
        self.assertRaises(
            kas.FileFormatError, self.read_file,
            "malformed/key_len_outside_file.kas")

    def test_array_len_outside_file(self):
        self.assertRaises(
            kas.FileFormatError, self.read_file,
            "malformed/array_len_outside_file.kas")

    def test_bad_array_start(self):
        self.assertRaises(
            kas.FileFormatError, self.read_file,
            "malformed/bad_array_start_-8.kas")
        self.assertRaises(
            kas.FileFormatError, self.read_file,
            "malformed/bad_array_start_-1.kas")
        self.assertRaises(
            kas.FileFormatError, self.read_file,
            "malformed/bad_array_start_1.kas")
        self.assertRaises(
            kas.FileFormatError, self.read_file,
            "malformed/bad_array_start_8.kas")
    ...
    def test_all_types(self):
        ...
        for n in range(10):
            filename = "v1/all_types_{}_elements.kas".format(n)
